...
char rawcommand[MAX_LENGTH], *commands[MAX_LENGTH], *tokens[MAX_LENGTH], *infile, *outfile, *alternate_tokens[MAX_LENGTH];
...
void cleanstring(char* a){
	...
	for(int loc = 0; loc <= j; loc++) if(isspace(a[loc])) a[loc] = ' ';
	while(a[i] == ' ') i++;
	while(a[j] == ' ' && i <= j) j--;
...
int runandgetdestination(char *commands, int source, int first, int last){
	int destination, pipe_nums[2], pip, infile_num, outfile_num, m = 1, in = 0, out = 0, flag = 1;
	...
	pid = fork();
	if(pid == 0){
		if(source == 0 && first == 1 && last == 0) dup2(pipe_nums[1], 1);
		else if(source != 0 && first == 0 && last == 0) dup2(source, 0), dup2(pipe_nums[1], 1);
		else dup2(source, 0);
		...
		if(in || out){
			...
			int m = 1;
			if(in == 1) toks[0] = strtok(temp, "<");
			else toks[0] = strtok(temp, ">");
			if(out == 1) while((toks[m] = strtok(NULL, ">")) != NULL) m++;
			else while((toks[m] = strtok(NULL, "<")) != NULL) m++;
			if(in && out){
				cleanstring(toks[1]);
				cleanstring(toks[2]);
